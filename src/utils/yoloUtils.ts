// yoloUtils.ts - YOLO detection utilities
import * as ort from 'onnxruntime-web';

export interface YoloDetection {
  box: number[]; // [x1, y1, x2, y2]
  score: number;
  cls: number;
}

export interface YoloConfig {
  modelPath: string;
  inputShape: number[]; // [N, C, H, W]
  scoreThresh: number;
  nmsIouThresh: number;
}

export interface LetterboxInfo {
  scale: number;
  padX: number;
  padY: number;
  newW: number;
  newH: number;
}

// Math utilities
export function sigmoid(x: number): number {
  return 1 / (1 + Math.exp(-x));
}

export function xywh2xyxy(x: number, y: number, w: number, h: number): number[] {
  const x1 = x - w / 2;
  const y1 = y - h / 2;
  const x2 = x + w / 2;
  const y2 = y + h / 2;
  return [x1, y1, x2, y2];
}

export function bboxIoU(a: number[], b: number[]): number {
  const [ax1, ay1, ax2, ay2] = a;
  const [bx1, by1, bx2, by2] = b;
  const interX1 = Math.max(ax1, bx1);
  const interY1 = Math.max(ay1, by1);
  const interX2 = Math.min(ax2, bx2);
  const interY2 = Math.min(ay2, by2);
  const interW = Math.max(0, interX2 - interX1);
  const interH = Math.max(0, interY2 - interY1);
  const interA = interW * interH;
  const aA = Math.max(0, ax2 - ax1) * Math.max(0, ay2 - ay1);
  const bA = Math.max(0, bx2 - bx1) * Math.max(0, by2 - by1);
  const union = aA + bA - interA;
  return union <= 0 ? 0 : interA / union;
}

export function nms(boxes: number[][], scores: number[], iouThresh: number, topK = 100): number[] {
  const idxs = scores.map((s, i) => [s, i] as [number, number])
    .sort((a, b) => b[0] - a[0])
    .map(v => v[1]);
  const picked: number[] = [];
  let arr = idxs;
  while (arr.length) {
    const i = arr.shift()!;
    picked.push(i);
    if (picked.length >= topK) break;
    const rest: number[] = [];
    for (const j of arr) {
      if (bboxIoU(boxes[i], boxes[j]) < iouThresh) rest.push(j);
    }
    arr = rest;
  }
  return picked;
}

// Canvas utilities for YOLO preprocessing
export function letterboxToCanvas(
  srcCanvas: HTMLCanvasElement,
  dstCanvas: HTMLCanvasElement,
  dstW: number,
  dstH: number,
  fill = [114, 114, 114, 255]
): LetterboxInfo {
  const srcW = srcCanvas.width;
  const srcH = srcCanvas.height;
  const r = Math.min(dstW / srcW, dstH / srcH);
  const newW = Math.round(srcW * r);
  const newH = Math.round(srcH * r);
  const dw = Math.floor((dstW - newW) / 2);
  const dh = Math.floor((dstH - newH) / 2);

  const ctx = dstCanvas.getContext('2d', { willReadFrequently: true })!;
  ctx.fillStyle = `rgba(${fill[0]},${fill[1]},${fill[2]},${fill[3] / 255})`;
  ctx.fillRect(0, 0, dstW, dstH);
  ctx.imageSmoothingEnabled = true;
  ctx.imageSmoothingQuality = 'low';
  ctx.drawImage(srcCanvas, 0, 0, srcW, srcH, dw, dh, newW, newH);
  return { scale: r, padX: dw, padY: dh, newW, newH };
}

export function preprocessForYolo(
  srcCanvas: HTMLCanvasElement,
  yoloCanvas: HTMLCanvasElement,
  yoloInputShape: number[]
): { input: ort.Tensor; info: LetterboxInfo } {
  const [n, c, H, W] = yoloInputShape;
  const info = letterboxToCanvas(srcCanvas, yoloCanvas, W, H);
  const ctx = yoloCanvas.getContext('2d', { willReadFrequently: true })!;
  const imageData = ctx.getImageData(0, 0, W, H);
  const data = imageData.data; // RGBA
  const chw = new Float32Array(n * c * H * W);
  const wh = W * H;
  for (let i = 0; i < wh; i++) {
    const r = data[i * 4] / 255;
    const g = data[i * 4 + 1] / 255;
    const b = data[i * 4 + 2] / 255;
    chw[i] = r;
    chw[i + wh] = g;
    chw[i + 2 * wh] = b;
  }
  const input = new ort.Tensor('float32', chw, [n, c, H, W]);
  return { input, info };
}

export function postprocessYolo(
  outTensor: ort.Tensor,
  info: LetterboxInfo,
  origW: number,
  origH: number,
  yoloInputShape: number[],
  yoloScoreThresh: number,
  yoloNmsIouThresh: number
): YoloDetection[] {
  const data = outTensor.data as Float32Array;
  const dims = outTensor.dims;
  const boxes: number[][] = [];
  const scores: number[] = [];
  const classes: number[] = [];

  const gain = info.scale;
  const padX = info.padX;
  const padY = info.padY;

  if (dims.length === 3) {
    const a = dims[1];
    const b = dims[2];
    if (b >= 6 && b <= 256) {
      // [1, N=a, no=b] (YOLOv5 style)
      const N = a;
      const no = b;
      const numClasses = no - 5;
      let needSigmoid = false;
      const probe = Math.min(10, N);
      for (let k = 0; k < probe; k++) {
        if (data[k * no + 4] > 1) {
          needSigmoid = true;
          break;
        }
      }
      for (let i = 0; i < N; i++) {
        const off = i * no;
        const cx = data[off + 0];
        const cy = data[off + 1];
        const w = data[off + 2];
        const h = data[off + 3];
        let obj = data[off + 4];
        if (needSigmoid) obj = sigmoid(obj);
        let best = 0;
        let bestIdx = 0;
        for (let c = 0; c < numClasses; c++) {
          let v = data[off + 5 + c];
          if (needSigmoid) v = sigmoid(v);
          if (v > best) {
            best = v;
            bestIdx = c;
          }
        }
        const conf = obj * (numClasses > 0 ? best : 1);
        if (conf < yoloScoreThresh) continue;
        let [x1, y1, x2, y2] = xywh2xyxy(cx, cy, w, h);
        x1 = (x1 - padX) / gain;
        y1 = (y1 - padY) / gain;
        x2 = (x2 - padX) / gain;
        y2 = (y2 - padY) / gain;
        x1 = Math.max(0, Math.min(origW - 1, x1));
        y1 = Math.max(0, Math.min(origH - 1, y1));
        x2 = Math.max(0, Math.min(origW - 1, x2));
        y2 = Math.max(0, Math.min(origH - 1, y2));
        boxes.push([x1, y1, x2, y2]);
        scores.push(conf);
        classes.push(bestIdx);
      }
    } else if (a >= 6 && a <= 256) {
      // [1, no=a, N=b] (YOLOv8 style)
      const no = a;
      const N = b;
      const numClasses = no - 4;
      let needSigmoid = false;
      const probe = Math.min(10, N);
      for (let k = 0; k < probe; k++) {
        if (data[4 * N + k] > 1) {
          needSigmoid = true;
          break;
        }
      }
      for (let i = 0; i < N; i++) {
        const cx = data[0 * N + i];
        const cy = data[1 * N + i];
        const w = data[2 * N + i];
        const h = data[3 * N + i];
        let best = 0;
        let bestIdx = 0;
        for (let c = 0; c < numClasses; c++) {
          let v = data[(4 + c) * N + i];
          if (needSigmoid) v = sigmoid(v);
          if (v > best) {
            best = v;
            bestIdx = c;
          }
        }
        const conf = best;
        if (conf < yoloScoreThresh) continue;
        let [x1, y1, x2, y2] = xywh2xyxy(cx, cy, w, h);
        x1 = (x1 - padX) / gain;
        y1 = (y1 - padY) / gain;
        x2 = (x2 - padX) / gain;
        y2 = (y2 - padY) / gain;
        x1 = Math.max(0, Math.min(origW - 1, x1));
        y1 = Math.max(0, Math.min(origH - 1, y1));
        x2 = Math.max(0, Math.min(origW - 1, x2));
        y2 = Math.max(0, Math.min(origH - 1, y2));
        boxes.push([x1, y1, x2, y2]);
        scores.push(conf);
        classes.push(bestIdx);
      }
    }
  }

  const keep = nms(boxes, scores, yoloNmsIouThresh, 20);
  return keep.map(i => ({ box: boxes[i], score: scores[i], cls: classes[i] }));
}

// Initialize YOLO session
export async function initYoloSession(modelPath: string): Promise<ort.InferenceSession> {
  try {
    if (ort.env && ort.env.wasm) {
      ort.env.wasm.wasmPaths = 'https://cdn.jsdelivr.net/npm/onnxruntime-web@1.23.0/dist/';
      ort.env.wasm.simd = true;
      const coi = typeof crossOriginIsolated !== 'undefined' && crossOriginIsolated;
      ort.env.wasm.numThreads = coi ? Math.min(4, navigator.hardwareConcurrency || 4) : 1;
    }
  } catch (e) {
    console.warn('Failed to configure ORT WASM:', e);
  }

  const ep: string[] = [];
  const gl = document.createElement('canvas').getContext('webgl2') || 
              document.createElement('canvas').getContext('webgl');
  if ('gpu' in navigator) ep.push('webgpu');
  if (gl) ep.push('webgl');
  ep.push('wasm');

  const so = { executionProviders: ep, graphOptimizationLevel: 'all' as const };
  const session = await ort.InferenceSession.create(modelPath, so);
  return session;
}

// Run YOLO inference on canvas
export async function runYoloOnCanvas(
  srcCanvas: HTMLCanvasElement,
  ortSession: ort.InferenceSession,
  yoloCanvas: HTMLCanvasElement,
  yoloInputShape: number[],
  yoloScoreThresh: number,
  yoloNmsIouThresh: number
): Promise<YoloDetection[]> {
  const { input, info } = preprocessForYolo(srcCanvas, yoloCanvas, yoloInputShape);
  const feeds: Record<string, ort.Tensor> = {};
  feeds[ortSession.inputNames[0]] = input;
  const output = await ortSession.run(feeds);
  const first = output[Object.keys(output)[0]];
  const dets = postprocessYolo(
    first,
    info,
    srcCanvas.width,
    srcCanvas.height,
    yoloInputShape,
    yoloScoreThresh,
    yoloNmsIouThresh
  );
  return dets;
}
